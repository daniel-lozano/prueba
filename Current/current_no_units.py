...
EV=np.linspace(0,3E-3,300)/(2*Do)#(-50E15,10E15,1000)*2*e#applied potential in eV
...
for j in range(len(T)):
    ...
    for i in range(len(EV)):
        ...
        J2[i]=N*area*C1(Vo,beta)*np.exp(beta*(C2(Vo,beta)*eta+mu))*(1-np.exp(-beta*EV[i]*(2*Do)))/(1-C2(Vo,beta))
