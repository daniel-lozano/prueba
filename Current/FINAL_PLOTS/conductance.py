...
EV=np.linspace(-0,1.5E-3,300)#/(2*Do)#(-50E15,10E15,1000)*2*e#applied potential in eV
print(EV)
...
for j in range(len(T)):
    ...
    for i in range(len(EV)):
        ...
        units=1#(2*DeT(T[j]))
        J2[i]=beta*(2*DeT(T[j]))*N*area*C1(Vo,beta)*np.exp(beta*(C2(Vo,beta)*eta+mu))*(np.exp(-beta*EV[i]*units))/(1-C2(Vo,beta))
    JTOT=list(J2[-1::-1])
    XTOT=list(-EV[-1::-1])
    JTOT.extend(J2)
    XTOT.extend(EV)
...
plt.ylabel("$ \\frac{dJ}{dV} $ (approx) ",size=15)
