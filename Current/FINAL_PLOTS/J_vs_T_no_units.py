...
area=1#346*375 #microns squared

conver=1E8#angstroms to microns


EV=np.linspace(5E-4,1E-3,8)#[0.0005/(2*Do),0.001/(2*Do),0.003/(2*Do),0.005/(2*Do)]#(-50E15,10E15,1000)*2*e#applied potential in eV
...
for k in range(len(EV)):
    ...
    for j in range(len(T)):
        ...
        units=1#2*Do
        J2[j]=N*area*C1(Vo,beta)*np.exp(beta*(C2(Vo,beta)*eta+mu))*(1-np.exp(-beta*EV[k]*units))/(1-C2(Vo,beta))

    plt.plot(T,J2,label="$ V= $"+str(round(EV[k],4)))
    print(J2[-1])
