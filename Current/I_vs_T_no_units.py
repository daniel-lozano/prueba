...
EV=[0.0005/(2*Do),0.001/(2*Do),0.003/(2*Do),0.005/(2*Do)]#(-50E15,10E15,1000)*2*e#applied potential in eV
...
for k in range(len(EV)):
    ...
    for j in range(len(T)):
        ...
        J2[j]=N*area*C1(Vo,beta)*np.exp(beta*(C2(Vo,beta)*eta+mu))*(1-np.exp(-beta*EV[k]*2*Do))/(1-C2(Vo,beta))

    plt.plot(T,J2,label="$ V= "+str(round(EV[k],3))+"2\Delta_0 $")
    print(J2[-1])
